...
main() {

	{
		...
		int ricks = 0;
		int mortys = 0;	
		for(int i=0; i<1543321; i++) {
			int r = rand() % 1000;
			if(r%2) {
				deq.push_back(&deq, MyClass{r, "Rick"});
				ricks++;
			}
			else {
				deq.push_front(&deq, MyClass{r, "Morty"});
				mortys++;
			}
		}
	
		//Deque_MyClass_print(&deq);
		printf("Sorting now.\n");
		...
		for(Deque_MyClass_Iterator it = deq.begin(&deq); !Deque_MyClass_Iterator_equal(it, deq.end(&deq)); it.inc(&it)) {
			if(strcmp("Rick", it.deref(&it).name) == 0)  r++;
			else m++;
			if(it.deref(&it).id < prev) {
				printf("It aint sorted....\n");
				break;
			}
		}

		if(r!=ricks || m!= mortys) printf("ShieBe %i,%i -> %i,%i\n", ricks, mortys, r,m);
		else printf("Super sugoi!!!!@!1!\n");
...

	{
		...
		for(int i=0; i<1492; i++) {
			char c = 97 + ( std::rand() % ( 122 - 97 + 1 ) );
			MyClass m = MyClass{i, "pikachu"};
			m.name[0] = c;
			if(i%2) deq.push_front(&deq, m);
			else deq.push_back(&deq, m);
		}
	...
	//	//printf("deq1, iter2=%i, iter1=%i\n", iter2.index, iter1.index);
    //    //printf("deq1\n");
	//	//Deque_int_print(&deq1);
	//	
